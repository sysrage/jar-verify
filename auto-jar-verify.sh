...
for BLDCONFIG in ${JAR_CFGDIR}/auto-verify-cfg-*.cfg; do (
  # Reset variables for each iteration and set defaults
  JAR_RELEASENAME=""
  JAR_RELEASENUM=""
  JAR_BUNAME=""
  JAR_RELTYPE=""
  ...
  JAR_FW_ZIP="Palau_@JAR_BUILDNUM@_LNVOSUPS_FW_Internal.zip"
  JAR_LNX_ZIP="Palau_@JAR_BUILDNUM@_LNVOSUPS_LNX_KIT_Internal.zip"
  JAR_WIN_ZIP="Palau_@JAR_BUILDNUM@_LNVOSUPS_WIN_Internal.zip"
  ...
  for i in ${JAR_NEWBUILDS}; do
    ...
    JAR_LOGFILE="/tmp/jar-verify-results-${JAR_RELTYPE}-${JAR_BUILDNUM}.txt"
    if [ -f "${JAR_BUILDDIR}/${JAR_BUILDNUM}/ReleaseNotes-${JAR_BUILDNUM}.html" ]; then
      # Build is complete
      if [ -f "${JAR_BUILDDIR}/${JAR_BUILDNUM}/packages/External/${JAR_EXT_ZIP/@JAR_BUILDNUM@/$JAR_BUILDNUM}" ] || [ -f "${JAR_BUILDDIR}/${JAR_BUILDNUM}/packages/Internal/${JAR_FW_ZIP/@JAR_BUILDNUM@/$JAR_BUILDNUM}" ] || [ -f "${JAR_BUILDDIR}/${JAR_BUILDNUM}/packages/Internal/${JAR_LNX_ZIP/@JAR_BUILDNUM@/$JAR_BUILDNUM}" ] || [ -f "${JAR_BUILDDIR}/${JAR_BUILDNUM}/packages/Internal/${JAR_WIN_ZIP/@JAR_BUILDNUM@/$JAR_BUILDNUM}" ]; then
        ...
        if [ ! -d "${JAR_WORKDIR}/${JAR_BUILDNUM}/" ]; then
          ...
          if [ -f "${JAR_BUILDDIR}/${JAR_BUILDNUM}/packages/External/${JAR_EXT_ZIP/@JAR_BUILDNUM@/$JAR_BUILDNUM}" ]; then
            # Lenovo package exists - unzip JAR files
            unzip -qq "${JAR_BUILDDIR}/${JAR_BUILDNUM}/packages/External/${JAR_EXT_ZIP/@JAR_BUILDNUM@/$JAR_BUILDNUM}" *${JAR_RELTYPE}/*.jar *${JAR_RELTYPE}/triggerfile -d "${JAR_WORKDIR}/${JAR_BUILDNUM}/" 2>&1 | grep -v 'caution: filename not matched' >> ${JAR_LOGFILE}
            ...
            if [ -f "${JAR_LOGFILE}" ] && [ "$(cat ${JAR_LOGFILE})" != "" ]; then
              ...
              if [[ "${JAR_NOJARS}" == "" ]]; then
                mail -s "[${JAR_BUNAME}/${JAR_RELTYPE}] JAR Verification For ${JAR_RELEASENAME} Build ${JAR_BUILDNUM} -- ${JAR_RESULTS}" "${JAR_EMAILTO}" ${JAR_EMAILEXTRAS} < ${JAR_LOGFILE}
              fi
            ...
            if [ -f "${JAR_BUILDDIR}/${JAR_BUILDNUM}/packages/Internal/${JAR_FW_ZIP/@JAR_BUILDNUM@/$JAR_BUILDNUM}" ]; then
              unzip -qq "${JAR_BUILDDIR}/${JAR_BUILDNUM}/packages/Internal/${JAR_FW_ZIP/@JAR_BUILDNUM@/$JAR_BUILDNUM}" -d "${JAR_WORKDIR}/${JAR_BUILDNUM}/"
            fi

            # Unzip Linux driver JARs from internal package if it exists
            if [ -f "${JAR_BUILDDIR}/${JAR_BUILDNUM}/packages/Internal/${JAR_LNX_ZIP/@JAR_BUILDNUM@/$JAR_BUILDNUM}" ]; then
              unzip -qq "${JAR_BUILDDIR}/${JAR_BUILDNUM}/packages/Internal/${JAR_LNX_ZIP/@JAR_BUILDNUM@/$JAR_BUILDNUM}" -d "${JAR_WORKDIR}/${JAR_BUILDNUM}/"
            fi

            # Unzip Windows driver JARs from internal package if it exists
            if [ -f "${JAR_BUILDDIR}/${JAR_BUILDNUM}/packages/Internal/${JAR_WIN_ZIP/@JAR_BUILDNUM@/$JAR_BUILDNUM}" ]; then
              unzip -qq "${JAR_BUILDDIR}/${JAR_BUILDNUM}/packages/Internal/${JAR_WIN_ZIP/@JAR_BUILDNUM@/$JAR_BUILDNUM}" -d "${JAR_WORKDIR}/${JAR_BUILDNUM}/"
            fi
  ...
  for i in ${JAR_NEWOCSABUILDS}; do
    ...
    JAR_LOGFILE="/tmp/jar-verify-results-${JAR_RELTYPE}-${JAR_OCSABUILDNUM}.txt"
    ...
    if [ -f "${JAR_LOGFILE}" ]; then
      ...
      # E-mail jar-verify results
      mail -s "[${JAR_BUNAME}/${JAR_RELTYPE}] JAR Verification For ${JAR_RELEASENAME} Build ${JAR_OCSABUILDNUM} (${JAR_OCSABUILDVER}) -- ${JAR_RESULTS}" "${JAR_OCSAEMAILTO}" ${JAR_EMAILEXTRAS} < ${JAR_LOGFILE}
